 ...
 *
 */
public class ValueMainRunner implements MainInterface{
	...
	public static void main(String[] args) {
		...
		ValueMainRunner mainObj = new ValueMainRunner();
	...
	public void executeAnalysis(Dataset ds) {
		...
		long finishTime;
		startTime = System.currentTimeMillis();
		List<ValueTransaction2> valueList = convertToValueTransactions(ds);
		double sum =0;
	...
	public List<ValueTransaction2> convertToValueTransactions(Dataset ds) {
		List<ValueTransaction2> transactionList = null;
		if(null != ds && null != ds.getResults() && ds.getResults().length > 0) {
			transactionList = new ArrayList<>();
			for (JSONResult transactionData : ds.getResults()) {
				ValueTransaction2 nonValueTransaction = CommonUtils.convertToVT2(transactionData);
	...
	double processTransactions(List<ValueTransaction2> valueList, int divident) {
		double blackHole;
...
//		System.out.println(CommonUtils.generateLogMsg("Average Transaction Amount: "+avgTransactionAmt));
		List<ValueTransaction2> workList = new ArrayList<>(valueList);
...
//		System.out.println(CommonUtils.generateLogMsg("No. of transactions successfull are: "+numberOfCustomers));
	...
	private double computeAverageTransactionAmount(List<ValueTransaction2> valueList) {
		if(null != valueList && !valueList.isEmpty()) {
			double sum = 0;
			for(ValueTransaction2 valueTransaction : valueList) {
	...
	private double computeAverageProcessingFee(List<ValueTransaction2> valueList, float rate) {
		if(null != valueList && !valueList.isEmpty()) {
			double sum = 0;
			for(ValueTransaction2 valueTransaction : valueList) {
				double transactionAmt = valueTransaction.getPaymentInfo().getTransactionAmount();
	...
	private List<ValueTransaction2> updateTransactions(List<ValueTransaction2> workList, int rate) {
		if(null != workList && !workList.isEmpty()) {
			for(ValueTransaction2 valueTransaction : workList) {
				double transactionAmt = valueTransaction.getPaymentInfo().getTransactionAmount();
				double processingFee = transactionAmt*rate;
				PaymentInfo2 pInfo = new PaymentInfo2(valueTransaction.getPaymentInfo().getCustAccountBalance(),valueTransaction.getPaymentInfo().getTransactionDate(),valueTransaction.getPaymentInfo().getTransactionTime(),processingFee,valueTransaction.getPaymentInfo().getTransactionFeeRate(),false);
				valueTransaction.setFeeInfo(pInfo);
				if(valueTransaction.getPaymentInfo().getCustAccountBalance() >= processingFee) {
					PaymentInfo2 updatedPInfo = new PaymentInfo2(valueTransaction.getPaymentInfo().getCustAccountBalance(),valueTransaction.getPaymentInfo().getTransactionDate(),valueTransaction.getPaymentInfo().getTransactionTime(),processingFee,valueTransaction.getPaymentInfo().getTransactionFeeRate(),true);
	...
	private int computeNumberOfCustomers(List<ValueTransaction2> updateTransactions) {
		if(null != updateTransactions && !updateTransactions.isEmpty()) {
			int noOfCustomers = 0;
			for(ValueTransaction2 valueTransaction : updateTransactions) {
