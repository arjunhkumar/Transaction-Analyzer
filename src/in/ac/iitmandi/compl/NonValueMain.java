...
public class NonValueMain implements MainInterface{
	...
	public void executeAnalysis(Dataset ds) {
		...
		long finishTime;
		startTime = System.currentTimeMillis();
		List<NonValueTransaction> valueList = convertToNonValueTransactions(ds);
		double sum =0;
	...
	public List<NonValueTransaction> convertToNonValueTransactions(Dataset ds) {
		...
		if(null != ds && null != ds.getResults() && ds.getResults().length > 0) {
			...
			for (JSONResult transactionData : ds.getResults()) {
				NonValueTransaction nonValueTransaction = CommonUtils.convertToNVT(transactionData);
...
//		System.out.println(CommonUtils.generateLogMsg("Average Transaction Amount: "+avgTransactionAmt));
		List<NonValueTransaction> workList = new ArrayList<>(valueList);
...
//		System.out.println(CommonUtils.generateLogMsg("No. of transactions successfull are: "+numberOfCustomers));
	...
	private double computeAverageTransactionAmount(List<NonValueTransaction> valueList) {
	...
	private double computeAverageProcessingFee(List<NonValueTransaction> valueList, float rate) {
		if(null != valueList && !valueList.isEmpty()) {
			double sum = 0;
			for(NonValueTransaction valueTransaction : valueList) {
				double transactionAmt = valueTransaction.getPaymentInfo().getTransactionAmount();
	...
	private List<NonValueTransaction> updateTransactions(List<NonValueTransaction> workList, int rate) {
	...
	private int computeNumberOfCustomers(List<NonValueTransaction> updateTransactions) {
